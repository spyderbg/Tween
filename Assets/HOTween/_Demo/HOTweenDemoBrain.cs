...
public class HOTweenDemoBrain : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        HOTween.To(CubeTrans1, 4, new TweenParms()
            .Prop("position", new Vector3(-3, 6, 0), true)
        ...
        HOTween.To(CubeTrans2, 3, new TweenParms()
                ...
                .Prop("rotation", new Vector3(0, 1024, 0),
                    true) // Relative rotation tween (this way rotations higher than 360 can be applied)
        ...
        HOTween.To(this, 3,
            new TweenParms().Prop("SampleString", "Hello I'm a sample tweened string").Ease(EaseType.Linear)
                .Loops(-1, LoopType.Yoyo));

        // Tween the sample floating point number while creating TweenParms first,
        // and then assigning it to HOTween.
        var tweenParms =
            new TweenParms().Prop("SampleFloat", 27.5f).Ease(EaseType.Linear).Loops(-1, LoopType.Yoyo);
        ...
        var colorTo = CubeTrans3.GetComponent<Renderer>().material.color;
        colorTo.a = 0;
