namespace Holoville.HOTween.Core.Easing
{
    ...
    public static class Cubic
    {
        ...
        public static float EaseOut(
            ...
            float unusedPeriod)
        {
            return changeValue * (float)((time = (float)(time / (double)duration - 1.0)) * (double)time * time + 1.0) + startValue;
        }
            ...
            float unusedPeriod)
        {
            return (time /= duration * 0.5f) < 1.0
                ? changeValue * 0.5f * time * time * time + startValue
                : (float)(changeValue * 0.5 * ((time -= 2f) * (double)time * time + 2.0)) + startValue;
        }
    }
}