namespace Holoville.HOTween.Core.Easing
{
    ...
    public static class Quart
    {
        ...
        public static float EaseOut(
            ...
            float unusedPeriod)
        {
            return (float)(-(double)changeValue * ((time = (float)(time / (double)duration - 1.0)) * (double)time * time * time - 1.0)) + startValue;
        }
            ...
            float unusedPeriod)
        {
            return (time /= duration * 0.5f) < 1.0
                ? changeValue * 0.5f * time * time * time * time + startValue
                : (float)(-(double)changeValue * 0.5 * ((time -= 2f) * (double)time * time * time - 2.0)) + startValue;
        }
    }
}